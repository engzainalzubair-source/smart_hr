...
@section('hr-content')
    ...
    <div id="tab-dashboard" class="tab-pane">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div
                class="bg-gradient-to-r from-blue-500 to-blue-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Total Employees</p>
                <h2 class="text-3xl font-bold mt-1">{{ $totalEmployees ?? 125 }}</h2>
            </div>

            <div
                class="bg-gradient-to-r from-green-500 to-green-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Active Employees</p>
                <h2 class="text-3xl font-bold mt-1">{{ $activeEmployees ?? 118 }}</h2>
            </div>

            <div
                class="bg-gradient-to-r from-pink-500 to-pink-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Late Today</p>
                <h2 class="text-3xl font-bold mt-1">{{ $lateToday ?? 4 }}</h2>
            </div>

            <div
                class="bg-gradient-to-r from-indigo-500 to-indigo-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Present Today</p>
                <h2 class="text-3xl font-bold mt-1">{{ $presentToday ?? 110 }}</h2>
            </div>

            <div
                class="bg-gradient-to-r from-teal-500 to-cyan-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Attendance %</p>
                <h2 class="text-3xl font-bold mt-1">{{ $attendancePercentage ?? 0 }}%</h2>
            </div>

            <div
                class="bg-gradient-to-r from-yellow-500 to-amber-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">On Leave</p>
                <h2 class="text-3xl font-bold mt-1">{{ $onLeave ?? 6 }}</h2>
            </div>

            <div
                class="bg-gradient-to-r from-gray-500 to-gray-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Archived Employees</p>
                <h2 class="text-3xl font-bold mt-1">{{ $archivedEmployees ?? 0 }}</h2>
            </div>

            <div
                class="bg-gradient-to-r from-violet-500 to-purple-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Avg Performance</p>
                <h2 class="text-3xl font-bold mt-1">{{ number_format($avgPerformance ?? 82.5, 1) }}%</h2>
            </div>

            <div
                class="bg-gradient-to-r from-cyan-500 to-sky-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Total Departments</p>
                <h2 class="text-3xl font-bold mt-1">{{ $totalDepartments ?? 0 }}</h2>
            </div>

            <div
                class="bg-gradient-to-r from-rose-500 to-red-400 text-white rounded-xl p-5 shadow hover:shadow-lg transition">
                <p class="text-sm opacity-90">Pending Requests</p>
                <h2 class="text-3xl font-bold mt-1">{{ $pendingRequests ?? 3 }}</h2>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow mt-6">
            <h3 class="text-lg font-semibold mb-4 text-gray-700">Rewards / Penalties (12 months)</h3>
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div class="col-span-2">
                    <div class="relative h-48">
                        <canvas id="chartRewards"></canvas>
                    </div>
                </div>
                <div class="col-span-1">
                    <h4 class="text-sm font-medium mb-2">Top rewarded (12m)</h4>
                    <ul class="space-y-2">
                        @foreach ($topRewards ?? [] as $t)
                            <li class="flex justify-between items-center p-2 border rounded">
                                <div class="text-sm">{{ $t['name'] }}</div>
                                <div class="text-sm font-semibold text-green-600">{{ $t['count'] }}</div>
                            </li>
                        @endforeach
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            ...
            <div class="bg-white rounded-xl p-6 shadow">
                ...
                <div class="flex items-center justify-between mb-3">
                    <div class="text-sm text-gray-600">30-day Average: <span
                            class="font-semibold">{{ number_format($avgPerformance30d ?? 0, 1) }}%</span></div>
                    @if (isset($topPerformerName))
                        <div class="text-sm text-gray-600">Top Performer: <span
                                class="font-semibold">{{ $topPerformerName }} ({{ $topPerformerAvg }}%)</span></div>
                    @endif
                </div>
                <div class="relative h-48">
        ...
        <div class="bg-white rounded-xl p-6 shadow mt-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <form method="GET" action="{{ route('hr.employees.create') }}" class="inline-fetch">
                    ...
                    <button type="submit" data-tab="employees" data-inline="true"
                        class="block w-full rounded-lg p-5 bg-gradient-to-r from-indigo-500 to-indigo-400 text-white hover:opacity-90">Add
                        Employee</button>
                </form>
                <form method="GET" action="{{ route('hr.attendances.create') }}" class="inline-fetch">
                    <input type="hidden" name="inline" value="1" />
                    <button type="submit" data-tab="attendance" data-inline="true"
                        class="block w-full rounded-lg p-5 bg-green-500 text-white hover:opacity-90">Record
                        Attendance</button>
                </form>
                <form method="GET" action="{{ route('hr.rewards.create') }}" class="inline-fetch">
                    <input type="hidden" name="inline" value="1" />
                    <button type="submit" data-tab="rewards" data-inline="true"
                        class="block w-full rounded-lg p-5 bg-pink-500 text-white hover:opacity-90">Add
                        Reward/Penalty</button>
                </form>
            </div>
        </div>
                    ...
                    @foreach ($presentEmployees as $p)
                        <div class="flex items-center justify-between p-2 border rounded">
                            <div>
                                ...
                                <div class="text-sm text-gray-500">{{ optional($p->department)->name }}</div>
                            </div>
                            <div class="text-sm text-gray-600">{{ $p->today_check_in ?? '' }}</div>
                        </div>
                    @endforeach
                </form>
            </div>
    ...
    {{-- Other modules moved to their own pages (employees, attendance, performance, rewards, departments, settings)
                 Use the sidebar links to navigate. The dedicated controllers return the appropriate views/partials. --}}
@endsection
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function showToast(message, type = 'info') {
                ...
                toast.className =
                    `fixed bottom-6 left-6 z-60 text-white px-4 py-2 rounded shadow ${colors[type] || colors.info}`;
            ...
            async function fetchAndShowForm(url, tab, inline) {
                try {
                    ...
                    try {
                        ...
                        if (form) {
                            injected = '';
                            const heading = form.previousElementSibling && (form.previousElementSibling
                                    .tagName === 'H2' || form.previousElementSibling.tagName === 'H3') ?
                    ...
                    if (injectedForm) {
                        injectedForm.addEventListener('submit', async (ev) => {
                            ...
                            const method = (injectedForm.getAttribute('method') || 'POST')
                                .toUpperCase();
                            const fetchOpts = {
                                method: method,
                                body: fm,
                            ...
                            const resp = await fetch(action, Object.assign(fetchOpts, {
                                credentials: 'same-origin'
                            }));
                            if (resp.status === 419) {
                                modalBody.innerHTML =
                                    '<div class="p-4 text-red-600">Session expired. Please reload the page and sign in again.</div>';
                                return;
                            }
                            if (resp.ok) {
            ...
            document.addEventListener('click', async (e) => {
                ...
                const url = originalUrl && originalUrl.includes('?') ? originalUrl + '&inline=1' : (
                    originalUrl || '') + '?inline=1';
            ...
            document.addEventListener('submit', async (e) => {
                ...
                const hasInline = url.includes('inline=1') || new URL(url, window.location.origin)
                    .searchParams.has('inline');
                if (!hasInline) {
                    url = url.includes('?') ? url + '&inline=1' : url + '?inline=1';
                }
                const tab = form.dataset.tab || (form.querySelector('button') && form.querySelector(
                    'button').dataset.tab);
                const inline = form.dataset.inline === 'true' || (form.querySelector('button') && form
                    .querySelector('button').dataset.inline === 'true');
                await fetchAndShowForm(url, tab, inline);
            });
            ...
            document.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (resp.status === 419) {
                        showToast('Session expired. Please reload the page and sign in again.',
                            'error');
                        return;
                    }
                    if (resp.ok) {
            ...
            document.addEventListener('change', (e) => {
                ...
                if (dot) {
                    if (checked) dot.style.transform = 'translateX(1.25rem)';
            ...
            async function refreshTab(tab) {
                try {
                    ...
                    const url = urlMap[tab] ? (urlMap[tab] + (urlMap[tab].includes('?') ? '&inline=1' :
                        '?inline=1')) : null;
                    if (!url) return;
                    const res = await fetch(url, {
                        headers: {
