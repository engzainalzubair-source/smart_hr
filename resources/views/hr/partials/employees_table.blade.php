<div class="bg-white p-4 rounded shadow">
    ...
    <h2 class="text-lg font-semibold">الموظفين</h2>
    <form method="GET" action="{{ route('hr.employees.create') }}" class="inline-fetch" data-tab="employees" data-inline="true">
        <input type="hidden" name="inline" value="1" />
        <button type="submit" class="px-3 py-1 bg-blue-600 text-white rounded">إضافة موظف</button>
    </form>
    </div>
                ...
                <tr class="border-t">
                    ...
                    <td>
                        @if(in_array($emp->id, $todayPresentIds ?? []))
                            <span class="px-2 py-1 text-sm bg-green-100 text-green-800 rounded">حاضر</span>
                        @else
                            <span class="px-2 py-1 text-sm bg-gray-100 text-gray-700 rounded">غير مسجل</span>
                        @endif
                    </td>
                    <td class="space-x-2">
                        @if($emp->trashed())
                            <span class="px-2 py-1 text-sm bg-gray-200 text-gray-700 rounded">مؤرشف</span>
                            <form method="POST" action="{{ route('hr.employees.restore', $emp->id) }}" class="inline-action inline-block" data-tab="employees">
                                @csrf
                                <button type="submit" class="text-green-600">استعادة</button>
                            </form>
                        @else
                            ...
                            <form method="POST" action="{{ route('hr.employees.archive', $emp) }}" class="inline-action inline-block" data-tab="employees">
                                @csrf
                                <button type="submit" class="text-rose-600">أرشفة</button>
                            </form>
                        @endif
                    </td>
                </tr>
