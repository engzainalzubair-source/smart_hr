...
<div class="bg-white p-4 rounded shadow">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
        <h2 class="text-lg font-semibold">المكافآت والعقوبات</h2>
        <div class="flex items-center gap-2">
            <a href="{{ route('hr.rewards.create') }}" class="px-3 py-1 bg-green-600 text-white rounded">إضافة</a>
            <a href="{{ route('hr.rewards.exportPdf') }}{{ request()->getQueryString() ? '?'.request()->getQueryString() : '' }}" class="px-3 py-1 bg-gray-700 text-white rounded">تصدير PDF</a>
    ...
    <div class="mb-4 grid grid-cols-1 lg:grid-cols-3 gap-4 items-center">
        ...
        <div>
            ...
            <input type="text" name="q" value="{{ request('q') }}" class="mt-1 block w-full border rounded px-2 py-1" placeholder="اسم أو بريد" />
        </div>
        <div>
            <label class="text-sm text-gray-600">النوع</label>
            <select name="type" class="mt-1 block w-full border rounded px-2 py-1">
                <option value="">الكل</option>
                <option value="reward" {{ request('type')=='reward'?'selected':'' }}>مكافأة</option>
                <option value="penalty" {{ request('type')=='penalty'?'selected':'' }}>عقوبة</option>
            </select>
        </div>
        <div>
        ...
        <div class="md:col-span-1">
            <button type="submit" class="px-3 py-1 bg-indigo-600 text-white rounded">فلتر</button>
        </div>
        </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="p-4 border rounded">
            <div class="text-sm text-gray-600">إجمالي المكافآت</div>
            <div class="text-2xl font-bold text-green-600">{{ number_format(optional($totals->get('reward'))->total ?? 0,2) }}</div>
            <div class="text-xs text-gray-500">عدد: {{ optional($totals->get('reward'))->count ?? 0 }}</div>
        </div>
        <div class="p-4 border rounded">
            ...
            <div class="text-xs text-gray-500">عدد: {{ optional($totals->get('penalty'))->count ?? 0 }}</div>
        </div>
        <div class="p-4 border rounded">
            <div class="text-sm text-gray-600">صافي (مكافآت - خصومات)</div>
            ...
            <div class="text-xs text-gray-500">نظرة سريعة حسب الفلاتر الحالية</div>
        </div>
    </div>
    <!-- Departments breakdown -->
    <div class="mb-4 p-4 border rounded bg-gray-50">
        <h4 class="text-sm font-medium mb-2">توزيع حسب الأقسام (ملخص)</h4>
        @php
            $deptMap = [];
            foreach($byDept ?? [] as $r) {
                $d = $r->department ?? 'غير معروف';
                $type = $r->type ?? 'reward';
                $amt = $r->total ?? 0;
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <div class="p-2 bg-white rounded border">
                ...
                <div class="text-xs text-green-700">مكافآت: {{ number_format($vals['reward'] ?? 0,2) }}</div>
                <div class="text-xs text-red-700">خصومات: {{ number_format($vals['penalty'] ?? 0,2) }}</div>
            </div>
            @endforeach
            @if(empty($top))
            <div class="text-sm text-gray-500">لا توجد بيانات كافية</div>
            @endif
        </div>
    </div>
    </section>
        ...
        <tbody>
            @foreach($items as $it)
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="max-w-xs">{{ Str::limit($it->reason, 60) }} @if($it->metadata) <button title="تفاصيل" onclick="document.getElementById('meta-{{ $it->id }}').classList.toggle('hidden')" class="text-xs text-indigo-600">تفاصيل</button>@endif
                        @if($it->metadata)
                    ...
                    <td class="space-x-1">
                        ...
                        <form method="POST" action="{{ route('hr.rewards.approve', $it) }}" class="inline">
                            @csrf
                            <button type="submit" class="px-2 py-1 bg-green-600 text-white rounded text-sm">موافقة</button>
                        </form>
                        @endif
                        <a href="{{ route('hr.rewards.edit', $it) }}" class="px-2 py-1 bg-gray-200 text-gray-800 rounded text-sm">تعديل</a>
                        <form method="POST" action="{{ route('hr.rewards.destroy', $it) }}" class="inline" onsubmit="return confirm('تأكيد الحذف؟');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="px-2 py-1 bg-red-600 text-white rounded text-sm">حذف</button>
                        </form>
                    </td>
                </tr>
            @endforeach
